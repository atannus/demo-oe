...
The two backends use different databases intentionally: NestJS uses MongoDB and FastAPI uses PostgreSQL. This choice serves two purposes. First, it makes the boundary between nodes concrete and visible — each node's storage is structurally distinct, not just logically partitioned. Second, it allows the outbox relay strategies to differ across the two nodes, demonstrating the same pattern with different underlying mechanics (see below). In a real deployment the databases would live on separate servers; the demo runs them on one machine for simplicity.
